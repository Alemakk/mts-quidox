...
export default function AuthorizationTypeItem ({ data = null }) {
  ...
  return (
    <AuthorizationItem onClick={() => ['id', 'usb'].includes(type) ? (type === 'id' ? handleLoginWithSimECP() : handleLoginWithECP() ) : history.push('/login')} className={disabled && 'disabled'} disabled={disabled}>
